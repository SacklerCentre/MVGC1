function publish2html_old(www_dir,htmlfiles,copyStuff,procHtml)
...
www_docs_dir      = fullfile(www_dir,verstr);
www_docs_html_dir = fullfile(www_docs_dir,'html');
www_docs_imag_dir = fullfile(www_docs_dir,'images');
www_docs_docs_dir = fullfile(www_docs_dir,'docs');

makedir(www_dir);
makedir(www_docs_dir);
makedir(www_docs_html_dir);
makedir(www_docs_imag_dir);
makedir(www_docs_docs_dir);
...
if copyStuff
    ...
    copyfiles(src_maintainer_dir, www_dir,           'mvgc_poster.pdf');
    copyfiles(src_maintainer_dir, www_dir,           'lcbarnett.png');
    copyfiles(src_maintainer_dir, www_dir,           'akseth.png');
    copyfiles(src_maintainer_dir, www_dir,           'mvgc_schema.png');
    copyfiles(src_maintainer_dir, www_dir,           'index.html');
%   copyfiles(src_docs_dir,       www_docs_dir,      'index.html');
    copyfiles(src_docs_dir,       www_docs_docs_dir, 'mvgc_preprint.pdf');
    copyfiles(src_docs_html_dir,  www_docs_imag_dir,  '*.png');
end
...
for i = 1:n
    ...
    outfile = fullfile(www_docs_html_dir,htmlfile);
    ...
    expr   = '<a href="mailto:l.c.barnett@sussex.ac.uk">Lionel Barnett</a>';
    repstr = 'Lionel Barnett (l.c.barnett AT sussex.ac.uk)';
    str    = strrep(str,expr,repstr);
    
    expr   = '<a href="mailto:a.k.seth@sussex.ac.uk">Anil K. Seth</a>';
    repstr = 'Anil K. Seth (a.k.seth AT sussex.ac.uk)';
    str    = strrep(str,expr,repstr);

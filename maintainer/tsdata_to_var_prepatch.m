...
function [A,SIG,E] = tsdata_to_var_prepatch(X,p,regmode)
...
elseif strcmpi(regmode,'LWR') % LWR (Morf)
    ...
    % Initialisation corrected (many thanks to Gonzalo Camba-Mendez for the heads-up)
    %
    %XF = reshape(XX(:,1:k,k+1:m,:),kn,M);
    %XB = reshape(XX(:,1:k,k:m-1,:),kn,M);
    XF = reshape(XX(:,1,1:m,:),n,N*m);
    XB = XF;

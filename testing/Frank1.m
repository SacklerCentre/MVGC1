%% MVGC demo
%
% Demonstrates the typical usage of the MVGC toolbox on generated VAR data for a
...
%%%Load data file that has you import from a .txt file, in matlab, as a tab delimited
%%% numeric matrix named 'input' (or whatever) and saved in current folder
%%% (as a .mat matrix). NAME MUST START WITH A LETTER.

clear all;

%load('habituation.mat'); % load data file
%X = x176to1176n9amygfgofcomniGLMnofilter; % must match loaded .mat file name above (unless loaded mat file is workspace of multiple data files)

load ~/misc/alltrialsdiv1000grangeramycalfglocofc.mat % ... or wherever your data is
X = alltrialsdiv1000grangeramycalfglocofc;            % ... or whatever it's called
...
morder    = 3;  % model order to use ('actual', 'AIC', 'BIC' or supplied numerical value)

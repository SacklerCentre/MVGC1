...
function [A,SIG,E] = tsdata_to_var_alt(X,p,regmode)
...
elseif strcmpi(regmode,'LWR') % LWR (Morf)
    ...
    CXF = sqrtpdm(XF*XF');
    if isempty(CXF), return; end % show-stopper!

    CXB = sqrtpdm(XB*XB');
    if isempty(CXB), return; end % show-stopper!
    ...
    while k <= p
        ...
        CEF = sqrtpdm(EF*EF');
        if isempty(CEF), return; end % show-stopper!

        CEB = sqrtpdm(EB*EB');
        if isempty(CEB), return; end % show-stopper!

        R = CEF'\(EF*EB')/CEB; % normalised reflection coefficients

        RF = sqrtpdm(I-R*R');
        if isempty(RF), return; end % show-stopper!

        RB = sqrtpdm(I-R'*R);
        if isempty(RB), return; end % show-stopper!
...
function X = sqrtpdm(A) % Matrix square root: assumes A positive-definite!
    
[Q,D] = eig(A); % could also use [Q,D] = schur(A,'complex');

D = diag(D);

if any(D <= 0) % not positive-definite!
    X = [];
    return
end

X = real(Q*diag(sqrt(D))*Q'); % in theory we don't need the "real"

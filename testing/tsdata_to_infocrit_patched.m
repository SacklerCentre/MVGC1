...
function [aic,bic,moaic,mobic] = tsdata_to_infocrit_patched(X,morder,regmode,verb)
...
% PATCHED (in case of unforseen failure)
moaic = NaN;
mobic = NaN;
...
elseif strcmpi(regmode,'LWR') % LWR (Morf)
    ...
    assert(cholp == 0,'initialisation failed (''forward'' covariance matrix not positive definite)'); % it's a show-stopper!

    [CXB,cholp] = chol(XB*XB');
    assert(cholp == 0,'initialisation failed (''backward'' covariance matrix not positive definite)'); % it's a show-stopper!
    ...
    [iw,~,wid]= warn_test(owstate,[],false);
    if iw % PATCHED
        error('initialisation failed (%s)\n',wid); % it's a show-stopper!
    end

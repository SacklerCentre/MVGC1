...
%
% If the regression is ill-conditioned then A will be "bad" (i.e. will contain a
% |NaN| or |Inf|; see <isbad.html |isbad|>) and _no warning is issued - the caller
% must check this condition!_ The caller should also check for a rank-deficient
% regression warning in OLS mode (these can be suppressed and tested for later
% with the <warn_supp.html |warn_supp|>, <warn_test.html |warn_test|> routines).
%
...
% coefficients define a stable VAR [1]. (This test is performed, along with
% other checks, in the routine <var_to_autocov.html |var_to_autocov|>, which
% will typically be called subsequent to this function.)
%
%% References
%
...
function [A,SIG,E] = tsdata_to_var1(X,p,regmode)
...
if  strcmpi(regmode,'OLS') % OLS (QR decomposition)
    ...
    if nargout > 1
        ...
        E   = reshape(E,n,m-p,N);  % put residuals back into per-trial form if necessary (does nothing if N == 1)
    end
...
elseif strcmpi(regmode,'LWR') % LWR (Morf)
    ...
    if nargout > 1
        E = AF(:,1:n)\EF;
        %E   = AF(:,1:n)\AF(:,kf)*reshape(XX(:,1:k,k+1:m,:),kn,M); % forward  prediction errors
        SIG = (E*E')/(M-1);       % residuals covariance matrix
        E   = reshape(E,n,m-p,N); % put residuals back into per-trial form if necessary (does nothing if N == 1)
    end
